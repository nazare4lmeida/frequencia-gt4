...
export default function Login({
  ...
  // Turmas disponíveis vêm do servidor, para refletir o que a coordenação
  // cadastrou e esconder turmas desativadas. Constants.js é o plano B.
  const [turmasDisponiveis, setTurmasDisponiveis] = useState(FORMACOES);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelado && dados.turmas?.length) {
          setTurmasDisponiveis(dados.turmas);
        }
      } catch {
        // Sem rede: seguimos com a lista embutida no código.
      }
    })();
  ...
  const handleUseAnotherAccount = () => {
    localStorage.removeItem("geracaotech_remember");
    setDadosSalvos(null);
  ...
  return (
    <div className="login-container">
      ...
      <div className="login-card">
        ...
        <p className="subtitle">Geração Tech 4.0</p>
        ...
        {dadosSalvos ? (
          <div className="welcome-back" style={{ textAlign: "center" }}>
            ...
            >
              Turma:{" "}
              {turmasDisponiveis.find((f) => f.id === dadosSalvos.formacao)?.nome ||
                "Não definida"}
            </p>
