  ...
  const salvarPerfil = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const sessionStr = localStorage.getItem("geracaotech_session");
        if (sessionStr) {
          const session = JSON.parse(sessionStr);
          session.userData = usuarioAtualizado;
          session.timestamp = Date.now();
          localStorage.setItem("geracaotech_session", JSON.stringify(session));
        }

        const rememberStr = localStorage.getItem("geracaotech_remember");
        if (rememberStr) {
